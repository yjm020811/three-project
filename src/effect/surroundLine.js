  ...
  createLine() {
    ...
    const material = new THREE.ShaderMaterial({
      uniforms: {
        ...
        u_max: { value: max },
        u_min: { value: min },
        live_color: { value: new THREE.Color(color.liveColor) }
      },
      vertexShader: `
        ...
        void main() {
          ...
          if (rangeY < position.y && rangeY > position.y - 200.0) {
            float f_index = 1.0 - sin((position.y - rangeY) / 200.0 * 3.1415926);
    ...
    // 动态更新 uniform 变量
    const updateLineUniforms = () => {
      material.uniforms.u_time.value = performance.now() / 1000;
      requestAnimationFrame(updateLineUniforms);
    };
    updateLineUniforms();
  }
}
